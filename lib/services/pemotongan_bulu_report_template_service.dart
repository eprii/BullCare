...
/// Export khusus aktivitas Pemotongan Bulu menggunakan formulir SOP-6.3f.
///
/// Service ini hanya dipanggil ketika seluruh record yang diekspor berasal
/// dari collection `pemotongan_bulu`. Formulir utama mengikuti formulir
/// yang benar dengan kolom Potong Bulu dan Keterangan. Halaman rincian
/// menjaga seluruh riwayat tetap tersedia bila dalam periode yang dipilih ada
/// lebih dari satu pemotongan bulu untuk bull yang sama. CRUD, Firestore,
/// dan modul aktivitas lain tidak disentuh.
class PemotonganBuluReportTemplateService {
  const PemotonganBuluReportTemplateService();

  static const String _wordTemplateAsset =
      'assets/templates/form_pemotongan_bulu_sop_6_3f.docx';
  static const String _pdfTemplateAsset =
      'assets/templates/form_pemotongan_bulu_sop_6_3f_page_1.png';
  ...
  // Batas 30 baris bull pada formulir SOP-6.3f.
  ...
  Future<Uint8List> buildPdf({
    ...
    final pw.Document document = pw.Document(
      title: 'Formulir Pemotongan Bulu SOP-6.3f',
      author: exportedBy.nama.trim().isEmpty ? exportedBy.email : exportedBy.nama,
      creator: 'BullCare',
    );
    final List<_PemotonganBuluBullRow> bulls = _bullSlots(data);
  ...
  String _patchWordTemplate(String xml, ReportExportData data) {
    ...
    if (tableMatches.length < 2) {
      throw StateError(
        'Template pemotongan bulu tidak valid: struktur tabel SOP-6.3f tidak lengkap.',
      );
    }
    ...
    // terisolasi hanya untuk ekspor pemotongan bulu.
  ...
  String _replaceTimeLabel(String xml, String label) {
    ...
    if (match == null) {
      throw StateError(
        'Template pemotongan bulu tidak valid: label Waktu tidak ditemukan.',
      );
    }
  ...
  String _patchWordDataTable(
    String tableXml, {
    required ReportExportData data,
    required List<_PemotonganBuluBullRow> bulls,
  }) {
    ...
    if (rowMatches.length < 32) {
      throw StateError(
        'Template pemotongan bulu tidak valid: 30 baris bull tidak lengkap.',
      );
    }
    ...
    for (int bullIndex = 0; bullIndex < _bullSlotCount; bullIndex++) {
      final _PemotonganBuluBullRow bull = bulls[bullIndex];
      final ActivityRecord? record =
          bull.id.isEmpty ? null : _latestHairCut(data, bull.id);
  ...
  String _replaceRowCellTexts(String rowXml, List<String> values) {
    ...
    if (cellMatches.length != values.length) {
      throw StateError(
        'Template pemotongan bulu tidak valid: jumlah kolom ${cellMatches.length}, '
  ...
  String _appendWordDetailSection(String xml, ReportExportData data) {
    final List<ActivityRecord> records = data.records
        .where((record) => record.collectionName == 'pemotongan_bulu')
    ...
    if (sectionIndex < 0) {
      throw StateError(
        'Template pemotongan bulu tidak valid: section Word tidak ditemukan.',
      );
    }
  ...
  pw.Widget _buildPdfTemplatePage({
    ...
    for (int bullIndex = 0; bullIndex < _bullSlotCount; bullIndex++) {
      ...
      final ActivityRecord? record = _latestHairCut(data, bull.id);
  ...
  List<_PemotonganBuluBullRow> _bullSlots(ReportExportData data) {
    final List<_PemotonganBuluBullRow> real = _bullRows(data);
    final List<_PemotonganBuluBullRow> result = real.take(_bullSlotCount).toList();
    while (result.length < _bullSlotCount) {
      result.add(const _PemotonganBuluBullRow(id: '', label: '', breed: ''));
    }
    return result;
  }

  List<_PemotonganBuluBullRow> _bullRows(ReportExportData data) {
    final Map<String, _PemotonganBuluBullRow> byId = <String, _PemotonganBuluBullRow>{};
    for (final BullModel bull in data.bulls.values) {
      final String name = bull.nama.trim().isNotEmpty
          ...
          : bull.kode_bull.trim().isNotEmpty
              ? bull.kode_bull.trim()
              : 'Bull';
      byId[bull.id] = _PemotonganBuluBullRow(
        id: bull.id,
        label: name,
    ...
    for (final ActivityRecord record in data.records) {
      if (record.collectionName != 'pemotongan_bulu') continue;
      byId.putIfAbsent(
        record.bull_id,
        () => _PemotonganBuluBullRow(
          id: record.bull_id,
    ...
    // formulir SOP-6.3f menuliskan "Pasifica". Keduanya ditempatkan di slot
    ...
    final List<_PemotonganBuluBullRow> values = byId.values.toList();
    values.sort((a, b) {
  ...
  ActivityRecord? _latestHairCut(ReportExportData data, String bullId) {
    ActivityRecord? latest;
    for (final ActivityRecord record in data.records) {
      if (record.collectionName != 'pemotongan_bulu' ||
...
class _PemotonganBuluBullRow {
  const _PemotonganBuluBullRow({
    required this.id,
    required this.label,
    required this.breed,
  });
