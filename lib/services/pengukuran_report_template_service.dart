...
/// Export khusus aktivitas Pengukuran menggunakan formulir SOP-6.3c.
///
/// Service ini hanya dipanggil ketika seluruh record yang diekspor berasal
/// dari collection `pengukuran`. Formulir utama mengikuti dokumen sumber
/// FORMULIR PENGUKURAN PEJANTAN (BULL), sedangkan halaman rincian menjaga
/// agar data tanggal, keterangan, dan petugas tetap tersedia bila pada
/// periode yang dipilih terdapat lebih dari satu pengukuran untuk bull yang
/// sama. CRUD, Firestore, dan modul aktivitas lain tidak disentuh.
class PengukuranReportTemplateService {
  const PengukuranReportTemplateService();

  static const String _wordTemplateAsset =
      'assets/templates/form_pengukuran_pejantan_sop_6_3c.docx';
  static const String _pdfTemplateAsset =
      'assets/templates/form_pengukuran_pejantan_sop_6_3c_page_1.png';
  ...
  // Batas kolom tabel hasil render template SOP-6.3c (piksel):
  // No, Nama Bull, Bangsa, Tinggi Gumba, Panjang Tubuh, Lingkar Badan,
  // Lingkar Skrotum, Keterangan.
  ...
  // Batas 30 baris bull. Header tabel berakhir pada sekitar y=546.
  ...
  static const double _signatureTopPx = 1371.5;
  static const double _signatureBottomPx = 1400.0;
  ...
  Future<Uint8List> buildPdf({
    ...
    final pw.Document document = pw.Document(
      title: 'Formulir Pengukuran Pejantan SOP-6.3c',
      author: exportedBy.nama.trim().isEmpty ? exportedBy.email : exportedBy.nama,
      creator: 'BullCare',
    );
    final List<_PengukuranBullRow> bulls = _bullSlots(data);
  ...
  String _patchWordTemplate(String xml, ReportExportData data) {
    ...
    if (tableMatches.length < 3) {
      throw StateError(
        'Template pengukuran tidak valid: struktur tabel SOP-6.3c tidak lengkap.',
      );
    }
  ...
  String _replaceTimeLabel(String xml, String label) {
    ...
    if (match == null) {
      throw StateError(
        'Template pengukuran tidak valid: label Waktu tidak ditemukan.',
      );
    }
  ...
  String _patchWordDataTable(
    String tableXml, {
    required ReportExportData data,
    required List<_PengukuranBullRow> bulls,
  }) {
    ...
    if (rowMatches.length < 33) {
      throw StateError(
        'Template pengukuran tidak valid: 30 baris bull tidak lengkap.',
      );
    }
    ...
    for (int bullIndex = 0; bullIndex < _bullSlotCount; bullIndex++) {
      final _PengukuranBullRow bull = bulls[bullIndex];
      final ActivityRecord? measurement =
          bull.id.isEmpty ? null : _latestMeasurement(data, bull.id);
      final List<String> values = <String>[
        '${bullIndex + 1}',
        bull.label,
        bull.breed,
        _measurementValue(measurement, 'tinggi_gumba'),
        _measurementValue(measurement, 'panjang_tubuh'),
        _measurementValue(measurement, 'lingkar_badan'),
        _measurementValue(measurement, 'lingkar_skrotum'),
        measurement == null ? '' : _plainValue(measurement.data['keterangan']),
      ];
    ...
    patchedRows[32] = _replaceRowCellTexts(
      ...
      <String>[
        'Paraf Petugas',
        for (int i = 0; i < 5; i++) i < petugas.length ? petugas[i] : '',
      ],
    );
  ...
  String _replaceRowCellTexts(String rowXml, List<String> values) {
    ...
    if (cellMatches.length != values.length) {
      throw StateError(
        'Template pengukuran tidak valid: jumlah kolom ${cellMatches.length}, '
  ...
  String _appendWordDetailSection(String xml, ReportExportData data) {
    final List<ActivityRecord> records = data.records
        .where((record) => record.collectionName == 'pengukuran')
    ...
    final StringBuffer appendix = StringBuffer()
      ..write(
        _wordParagraph(
          'RINCIAN DATA PENGUKURAN PEJANTAN',
          bold: true,
          center: true,
          fontSize: 20,
    ...
    final List<List<String>> rows = <List<String>>[
      ...
      for (final ActivityRecord record in records)
        <String>[
          ...
          _detailMeasurement(record.data['tinggi_gumba']),
          _detailMeasurement(record.data['panjang_tubuh']),
          _detailMeasurement(record.data['lingkar_badan']),
          _detailMeasurement(record.data['lingkar_skrotum']),
    ...
    if (sectionIndex < 0) {
      throw StateError(
        'Template pengukuran tidak valid: section Word tidak ditemukan.',
      );
    }
  ...
  pw.Widget _buildPdfTemplatePage({
    ...
    for (int bullIndex = 0; bullIndex < _bullSlotCount; bullIndex++) {
      ...
      // Tutup nama/bangsa bawaan formulir agar isi selalu mengikuti database.
      children.add(
        _pdfWhiteBox(
      ...
      if (bull.label.isNotEmpty) {
        children.add(
          _pdfTextBox(
            text: bull.label,
            leftPx: _columnBoundsPx[1] + 3,
            topPx: top,
            widthPx: (_columnBoundsPx[2] - _columnBoundsPx[1]) - 6,
            heightPx: height,
            fontSize: 5.0,
            align: pw.Alignment.centerLeft,
          ),
      ...
      if (bull.breed.isNotEmpty) {
        children.add(
          _pdfTextBox(
            text: bull.breed,
            leftPx: _columnBoundsPx[2] + 3,
            topPx: top,
            widthPx: (_columnBoundsPx[3] - _columnBoundsPx[2]) - 6,
            heightPx: height,
            fontSize: 4.7,
            align: pw.Alignment.centerLeft,
          ),
      ...
      final ActivityRecord? measurement = _latestMeasurement(data, bull.id);
      ...
      final List<String> values = <String>[
        ...
        _measurementValue(measurement, 'lingkar_badan'),
        _measurementValue(measurement, 'lingkar_skrotum'),
        _plainValue(measurement.data['keterangan']),
      ];
          ...
          _pdfTextBox(
            ...
            widthPx:
                (_columnBoundsPx[columnIndex + 1] - _columnBoundsPx[columnIndex]) -
                    4,
            heightPx: height,
            fontSize: i == 4 ? 4.2 : 5.0,
            align: i == 4 ? pw.Alignment.centerLeft : pw.Alignment.center,
          ),
        );
      }
    }
    ...
    final List<String> petugas = _petugasSlots(data);
    for (int i = 0; i < petugas.length && i < 5; i++) {
      final int columnIndex = i + 3;
      children.add(
        _pdfTextBox(
          text: petugas[i],
  ...
  List<pw.Widget> _pdfDetailSection(ReportExportData data) {
    ...
    return <pw.Widget>[
      ...
      pw.TableHelper.fromTextArray(
        ...
        headerStyle: pw.TextStyle(fontSize: 5.4, fontWeight: pw.FontWeight.bold),
  ...
  List<_PengukuranBullRow> _bullSlots(ReportExportData data) {
    final List<_PengukuranBullRow> real = _bullRows(data);
    final List<_PengukuranBullRow> result = real.take(_bullSlotCount).toList();
    while (result.length < _bullSlotCount) {
      result.add(const _PengukuranBullRow(id: '', label: '', breed: ''));
    }
    return result;
  }

  List<_PengukuranBullRow> _bullRows(ReportExportData data) {
    final Map<String, _PengukuranBullRow> byId = <String, _PengukuranBullRow>{};
    for (final BullModel bull in data.bulls.values) {
      final String name = bull.nama.trim().isNotEmpty
          ...
          : bull.kode_bull.trim().isNotEmpty
              ? bull.kode_bull.trim()
              : 'Bull';
      byId[bull.id] = _PengukuranBullRow(
        id: bull.id,
        label: name,
    ...
    for (final ActivityRecord record in data.records) {
      if (record.collectionName != 'pengukuran') continue;
      byId.putIfAbsent(
        record.bull_id,
        () => _PengukuranBullRow(
          id: record.bull_id,
    ...
    // formulir SOP-6.3c menuliskan "Pasifica". Keduanya ditempatkan di slot
    ...
    final List<_PengukuranBullRow> values = byId.values.toList();
    values.sort((a, b) {
  ...
  ActivityRecord? _latestMeasurement(ReportExportData data, String bullId) {
    ActivityRecord? latest;
    for (final ActivityRecord record in data.records) {
      if (record.collectionName != 'pengukuran' || record.bull_id != bullId) {
        continue;
      }
  ...
  String _measurementValue(ActivityRecord? record, String key) {
    if (record == null) return '';
    final double? value = _parseNumber(record.data[key]);
  ...
  String _detailMeasurement(dynamic value) {
    final double? parsed = _parseNumber(value);
    return parsed == null ? '-' : _formatNumber(parsed);
  }
...
class _PengukuranBullRow {
  const _PengukuranBullRow({
    required this.id,
    required this.label,
    required this.breed,
  });
