...
/// Export khusus aktivitas Pemotongan Kuku menggunakan formulir SOP-6.3g.
///
/// Service ini hanya dipanggil ketika seluruh record yang diekspor berasal
/// dari collection `pemotongan_kuku`. Formulir utama mengikuti formulir
/// yang benar dengan kolom Potong Kuku dan Keterangan. Halaman rincian
/// menjaga seluruh riwayat tetap tersedia bila dalam periode yang dipilih ada
/// lebih dari satu pemotongan kuku untuk bull yang sama. CRUD, Firestore,
/// dan modul aktivitas lain tidak disentuh.
class PemotonganKukuReportTemplateService {
  const PemotonganKukuReportTemplateService();

  static const String _wordTemplateAsset =
      'assets/templates/form_pemotongan_kuku_sop_6_3g.docx';
  static const String _pdfTemplateAsset =
      'assets/templates/form_pemotongan_kuku_sop_6_3g_page_1.png';
  ...
  // Batas kolom pada render 1414 x 2000: No, Nama Bull, Bangsa,
  // Potong Kuku, Keterangan.
  ...
  // Batas 30 baris bull pada formulir SOP-6.3g.
  static const List<double> _dataRowBoundsPx = <double>[
    528.0, 563.0, 598.0, 632.0, 667.0, 702.0, 737.0, 772.0,
    807.0, 841.0, 876.0, 911.0, 946.0, 980.0, 1015.0, 1050.0,
    1085.0, 1120.0, 1154.0, 1189.0, 1224.0, 1259.0, 1294.0,
    1328.0, 1363.0, 1398.0, 1433.0, 1467.0, 1502.0, 1537.0,
    1572.0,
  ];

  static const double _signatureTopPx = 1572.0;
  static const double _signatureBottomPx = 1596.0;
  ...
  Future<Uint8List> buildPdf({
    ...
    final pw.Document document = pw.Document(
      title: 'Formulir Pemotongan Kuku SOP-6.3g',
      author: exportedBy.nama.trim().isEmpty ? exportedBy.email : exportedBy.nama,
      creator: 'BullCare',
    );
    final List<_PemotonganKukuBullRow> bulls = _bullSlots(data);
  ...
  String _patchWordTemplate(String xml, ReportExportData data) {
    ...
    if (tableMatches.length < 2) {
      throw StateError(
        'Template pemotongan kuku tidak valid: struktur tabel SOP-6.3g tidak lengkap.',
      );
    }
    ...
    // terisolasi hanya untuk ekspor pemotongan kuku.
  ...
  String _replaceTimeLabel(String xml, String label) {
    ...
    if (match == null) {
      throw StateError(
        'Template pemotongan kuku tidak valid: label Waktu tidak ditemukan.',
      );
    }
  ...
  String _patchWordDataTable(
    String tableXml, {
    required ReportExportData data,
    required List<_PemotonganKukuBullRow> bulls,
  }) {
    ...
    if (rowMatches.length < 32) {
      throw StateError(
        'Template pemotongan kuku tidak valid: 30 baris bull tidak lengkap.',
      );
    }
    ...
    for (int bullIndex = 0; bullIndex < _bullSlotCount; bullIndex++) {
      final _PemotonganKukuBullRow bull = bulls[bullIndex];
      final ActivityRecord? record =
          bull.id.isEmpty ? null : _latestHoofTrim(data, bull.id);
      final List<String> values = <String>[
        '${bullIndex + 1}',
        bull.label,
        bull.breed,
        record == null ? '' : _cutValue(record.data['dipotong']),
  ...
  String _replaceRowCellTexts(String rowXml, List<String> values) {
    ...
    if (cellMatches.length != values.length) {
      throw StateError(
        'Template pemotongan kuku tidak valid: jumlah kolom ${cellMatches.length}, '
  ...
  String _appendWordDetailSection(String xml, ReportExportData data) {
    final List<ActivityRecord> records = data.records
        .where((record) => record.collectionName == 'pemotongan_kuku')
    ...
    final List<List<String>> rows = <List<String>>[
      ...
      for (final ActivityRecord record in records)
        <String>[
          ...
          _cutValue(record.data['dipotong'], emptyAsDash: true),
    ...
    if (sectionIndex < 0) {
      throw StateError(
        'Template pemotongan kuku tidak valid: section Word tidak ditemukan.',
      );
    }
  ...
  pw.Widget _buildPdfTemplatePage({
    ...
    for (int bullIndex = 0; bullIndex < _bullSlotCount; bullIndex++) {
      ...
      final ActivityRecord? record = _latestHoofTrim(data, bull.id);
      if (record == null) continue;

      final List<String> values = <String>[
        _cutValue(record.data['dipotong']),
    ...
    if (petugas.isNotEmpty) {
      children.add(
        _pdfTextBox(
          ...
          widthPx: (_columnBoundsPx.last - _columnBoundsPx[2]) - 4,
  ...
  List<_PemotonganKukuBullRow> _bullSlots(ReportExportData data) {
    final List<_PemotonganKukuBullRow> real = _bullRows(data);
    final List<_PemotonganKukuBullRow> result = real.take(_bullSlotCount).toList();
    while (result.length < _bullSlotCount) {
      result.add(const _PemotonganKukuBullRow(id: '', label: '', breed: ''));
    }
    return result;
  }

  List<_PemotonganKukuBullRow> _bullRows(ReportExportData data) {
    final Map<String, _PemotonganKukuBullRow> byId = <String, _PemotonganKukuBullRow>{};
    for (final BullModel bull in data.bulls.values) {
      final String name = bull.nama.trim().isNotEmpty
          ...
          : bull.kode_bull.trim().isNotEmpty
              ? bull.kode_bull.trim()
              : 'Bull';
      byId[bull.id] = _PemotonganKukuBullRow(
        id: bull.id,
        label: name,
    ...
    for (final ActivityRecord record in data.records) {
      if (record.collectionName != 'pemotongan_kuku') continue;
      byId.putIfAbsent(
        record.bull_id,
        () => _PemotonganKukuBullRow(
          id: record.bull_id,
    ...
    // formulir SOP-6.3g menuliskan "Pasifica". Keduanya ditempatkan di slot
    ...
    final List<_PemotonganKukuBullRow> values = byId.values.toList();
    values.sort((a, b) {
  ...
  ActivityRecord? _latestHoofTrim(ReportExportData data, String bullId) {
    ActivityRecord? latest;
    for (final ActivityRecord record in data.records) {
      if (record.collectionName != 'pemotongan_kuku' ||
  ...
  String _cutValue(dynamic value, {bool emptyAsDash = false}) {
    if (value == null) return emptyAsDash ? '-' : '';
    if (value is bool) return value ? 'Ya' : 'Tidak';
    final String text = value.toString().trim();
    if (text.isEmpty) return emptyAsDash ? '-' : '';
...
class _PemotonganKukuBullRow {
  const _PemotonganKukuBullRow({
    required this.id,
    required this.label,
    required this.breed,
  });
